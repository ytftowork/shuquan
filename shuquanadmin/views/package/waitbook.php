  ...
  for(var j=0;j<checklist.length;j++)
  {
     checklist[j].checked = 0;
		...
		<div class="row">
				...
				<?php ActiveForm::end(); ?>  		
<?php $form = ActiveForm::begin([  
			'action' => ['package/confirmall'], //提交地址(*可省略*)  
			'method'=>'post',    //提交方法(*可省略默认POST*)  
			'id' => 'login-form', //设置ID属性  
			'options' => [  
					'class' => 'form-horizontal', //设置class属性  
			...
			<table class="table" style="width:65%">  
				<thead>  
					<tr>  
						...
						<th style="width:100px;"><label>姓名</label></th>  
						<th style="width:120px;"><label>电话</label></th>  
						<th style="width:100px;"><label></label></th>
						<th style="width:100px;"><label></label></th>
						<th style="width:40px;"><label><button type="submit" class="delete btn btn-xs btn-primary">确认收书</button> </label></th>    
					</tr>  
				</thead>  
				<tbody style="border:#dddddd solid 1px;">
				<?php foreach($data as $v){?>
				<tr ><td colspan="5"><?php echo date('Y-m-d H:i:s',$v->time);?><strong style="margin-left:15px;">订单号：<?=$v->id?></strong></td><td colspan="2">取书时间：<?php echo date('Y-m-d',$v->time+24*3600);?>&nbsp以后</td></tr>
				...
				<?php if($b->package->status==2){?>
					<tr style="background-color:;">  
						...
						<td></td> 
						<td></td>
						<td class="operation"><?= Html::a("确认收书",['package/confirm', 'id' => $b->packageid],['class' => 'delete btn btn-xs btn-primary']); ?></td>  
					</tr> 
				<?php }?>					
				<?php }?>
				<?php }?>
